...
extern "C" {        
    ...
    void __global__ adj_try_phi(float *f, float *data, float *theta, float center, float* phi, int s_z, int n, int nz, int deth, int ntheta)
    {
        int tx = blockDim.x * blockIdx.x + threadIdx.x;
        int ty = blockDim.y * blockIdx.y + threadIdx.y;
        int tz = blockDim.z * blockIdx.z + threadIdx.z;
...
adj_try_phi_kernel = module.get_function('adj_try_phi')
...
def adj_try(f, data, theta, center, phi, s_z):
    ...
    adj_try_kernel((int(cp.ceil(n/32)), int(cp.ceil(n/32+0.5)), nz), (32, 32, 1),
               (f, data, theta, center, -cp.float32(phi), s_z, n, nz, deth, ntheta))#sign different
    return data

def adj_try_phi(f, data, theta, center, phi, s_z):
    [nz, n] = f.shape[:2]
    [ntheta, deth] = data.shape[:2]
    adj_try_phi_kernel((int(cp.ceil(n/32)), int(cp.ceil(n/32+0.5)), nz), (32, 32, 1),
               (f, data, theta, cp.float32(center), -phi, s_z, n, nz, deth, ntheta))
    return data
